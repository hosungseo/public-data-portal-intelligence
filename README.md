...
파일데이터는 한 번 여는 순간마다 구조를 다시 파악해야 쓸 수 있습니다. 사람도 그렇고, 그 뒤에 오는 자동화 파이프라인도 그렇습니다. API는 그 단계를 한 번만 치르는 형태로 줄인 제공방식입니다. 모든 파일을 API로 바꿔야 한다는 뜻은 아니지만, 수요가 반복되고 구조가 단순한 후보일수록 그 단계를 한 번만 치르게 만들 가치가 커집니다. 이 페이지는 어느 데이터에서 그 가치가 가장 크게 드러나는지를 현업이 스스로 훑어볼 수 있도록 돕습니다.
...
**01 · 포함 기준** — 어떤 행이 큐에 오르는가
- 공공데이터포털의 파일형 데이터
- 다운로드 수요 **1,000건 이상** **OR** **국가중점데이터(`국가중점여부 == "Y"`)**
- 목록·서비스 유형·데이터 형식에서 API형 패턴이 감지되지 않음

**02 · 검토 레인** — 어느 레인으로 분류되는가
...
**03 · 우선 검토 점수** — 상위 12건 shortlist는 어떻게 정해지는가
...
- **11,238** file-to-API candidates (검색·필터·페이지네이션 가능)
- **9,573** rows already attach universe, metadata, and usage
- **7,286** rows already expose response fields
- **2,839** rows already show some API-side demand
- **1,572** rows are 국가중점데이터 (정부 지정 우선순위)
